...
describe('TelegramChannel', () => {
  ...
  describe('non-text messages', () => {
    ...
    it('stores voice message with transcription', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      // Mock fetch API for downloading the file
      ...
      // Get the bot and mock its API
      const bot = currentBot();
      bot.api.getFile = vi.fn().mockResolvedValueOnce({
        file_path: 'voice/voice_123.ogg',
      });
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({
          content: '[Voice: Hello, this is a test voice message.]',
        }),
      );

      // Reset mocks
      vi.mocked(transcribeAudioBuffer).mockReset();
