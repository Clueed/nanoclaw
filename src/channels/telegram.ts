  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message:voice', async (ctx) => {
      ...
      try {
        ...
        if (!response.ok) {
          logger.warn(
            { chatJid, status: response.status },
        ...
        logger.info(
          ...
          'Downloaded Telegram voice message for transcription',
        );

        // Transcribe using the shared transcription function
        const transcript = await transcribeAudioBuffer(buffer);

        // Format matches WhatsApp style: [Voice: transcript]
        // If transcription failed/unavailable, transcript contains fallback message
        this.opts.onMessage(chatJid, {
          id: ctx.message.message_id.toString(),
          chat_jid: chatJid,
          sender: ctx.from?.id?.toString() || '',
        ...
        logger.info(
          { chatJid, sender: senderName, transcriptLength: transcript?.length },
